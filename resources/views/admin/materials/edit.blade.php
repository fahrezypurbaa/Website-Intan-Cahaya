                ...
                <select name="training_id" class="w-full border p-2 rounded" required @change="updateCategory($event)">
                    @foreach ($trainings as $training)
                        <option value="{{ $training->id }}" 
                            data-category="{{ $training->category->name ?? '' }}"
                ...
                <template x-if="!category.includes('kemnaker') && category !== ''">
                    <div class="grid grid-cols-12 gap-2">
                        <div class="col-span-4">
                            ...
                            <input type="text" 
                                name="kode_unit" 
                                value="{{ old('kode_unit', $material->kode_unit) }}"
                                placeholder="Misal: M.71KKK01.001.1"
                                class="w-full border p-2 rounded">
                        </div>
                        <div class="col-span-8">
                            ...
                            <input type="text" 
                                name="title" 
                                value="{{ old('title', $material->title) }}"
                                placeholder="Judul Uji Kompetensi"
                                class="w-full border p-2 rounded" required>
                        </div>
                    </div>
                </template>
