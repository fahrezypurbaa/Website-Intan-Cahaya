...
@section('content')
    <x-admin.form-wrapper title="📚 Tambah Training" action="{{ route('admin.trainings.store') }}" method="POST"
        submit="✅ Simpan Training" back="{{ route('admin.trainings.index') }}" enctype="multipart/form-data">
        {{-- Kategori --}}
        <div>
        ...
        <div class="mb-4">
            ...
            <input type="text" id="slug" name="slug" class="w-full border rounded p-2 bg-gray-100 text-gray-600"
        ...
        <div>
            ...
            <input type="text" name="duration" class="w-full border rounded p-2" value="{{ old('duration') }}" required>
            @error('duration')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
